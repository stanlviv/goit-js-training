let text4 = `На рахунку користувача є 23580 кредитів, значення зберігається в змінній credits (створи і привласни). Користувач вирішує купити ремонтних дроїд, які коштують по 3000 кредитів за штуку. Ціна одного дроїда зберігається в змінній pricePerDroid (створи і привласни).
...
function task4() {
    ...
    let credits = 23580;
    let totalPrice;
    let response = prompt("How many droids to buy?");
    if (!response){
        console.log("Скасовано користувачем!");
    } else {
        totalPrice = (pricePerDroid * Number(response));
        let result = totalPrice <= credits && totalPrice > 0 ? `Ви купили ${response} дроїдів, на рахунку залишилося ${credits - totalPrice} кредитів.` : "Недостатньо коштів на рахунку!";
        console.log(result);
        alert(result)
    }
}